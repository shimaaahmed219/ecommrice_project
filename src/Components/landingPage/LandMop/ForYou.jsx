    ...
    <div className="py-10 w-full">
      <div className="grid sm:grid-cols-3 grid-cols-2 gap-x-5 px-3">
        {currentItems.map((item) => (
          <div key={item.id}>
            <img src={item.image} className=" hover:scale-[1.1] h-[180px] w-[180px]" alt={item.title} />
            <h6 className="font-light font-roboto mt-3 text-[12px]">
            ...
            <div>
              ...
              <span>({item.id})</span>{" "}
              <span className="ml-4 text-gray-500 text-[10px]">{"great service"}</span>
            </div>
            <div className="flex items-center pr-10">
            
              
              <MapsUgcIcon className="text-blue-500  mb-2 ms-auto" />
            </div>
          </div>
        ))}
      </div>
